...
function Get-CompatibleVisualStudios {
    ...
    if ($ErrorIfNotFound -and ($instances | Measure-Object).Count -eq 0) {
        ...
        $err = (
            ...
            "2. Run:`n"+
            "     winget install Microsoft.VisualStudio.2019.BuildTools --source winget --override `"--wait --passive --installPath $($pwd.drive.name):\VS --addProductLang En-us --add Microsoft.VisualStudio.Workload.VCTools --includeRecommended`"`n" +
            "`nSOLUTION 2`n"+
